...
local function requireModule(name: string): any?
    ...
    if not ok then warn("[JMC][Events] require(" .. name .. ") failed:", result); return nil end
    return result
end

local function getContext(): any
    if not ModulesMap.platform then ModulesMap.platform = requireModule("PlatformEngine") end
    if not ModulesMap.presence then ModulesMap.presence = requireModule("CirclePresence") end
    if not ModulesMap.reward   then ModulesMap.reward   = requireModule("RewardService") end
    if not ModulesMap.ragdoll  then ModulesMap.ragdoll  = requireModule("RagdollService") end
...
local function loadEvents()
    ...
    if Config.Debug.LogEvents then
        print(string.format("[JMC][Events] Загружено событий: %d (%s)",
            #eventOrder, table.concat(eventOrder, ", ")))
    end
end
...
local function pickNext(): string?
    ...
    if #eventOrder == 0 then return nil end

    -- Сначала пробуем выбрать из «свежих» (не в recent)
    local candidates: { string } = {}
    for _, n in ipairs(eventOrder) do
        if not isRecent(n) then table.insert(candidates, n) end
    end
    if #candidates == 0 then candidates = table.clone(eventOrder) end
    return candidates[ModulesMap.random:NextInteger(1, #candidates)]
end

local function runEvent(name: string)
    local def = events[name]
    if not def then warn("[JMC][Events] Неизвестное событие:", name); return end
    ...
    if type(def.Stop) == "function" then
        ...
        if not stopOk then warn("[JMC][Events] " .. name .. ":Stop():", stopErr) end
    end
...
local function mainLoop()
    while true do
        ...
        if Config.Debug.Enabled then wait = math.min(wait, 15) end
        task.wait(wait)
