...
--[[
    ...
    Зависимости: MonsterBodyService (для центра/радиуса).
--]]
...
local function getPlatform(): any
	...
	local mod = ServerScriptService:WaitForChild("Modules"):WaitForChild("MonsterBodyService")
...
local function isInsideCircle(player: Player): boolean
	...
	-- Засчитываем всех, кто находится внутри горизонтальных границ платформы,
	-- даже если игрок сейчас в воздухе над ней.
	...
	if hrp.Position.Y < surfaceY - 5 then
		return false
	end
...
function CirclePresence:Start()
	...
	task.spawn(function()
		...
		while task.wait(interval) do
			...
			if not ok then
				warn("[JMC][CirclePresence] tick error:", err)
			end
		end
	end)
...
--- Принудительно выбросить игрока из круга и сбросить непрерывный таймер.
--- Используется событиями, где игрок гарантированно срывается с платформы.
function CirclePresence:ForceExit(player: Player)
	local wasInside = inside[player] == true
	local ct = continuousTime[player] or 0
	continuousTime[player] = 0
	inside[player] = false
