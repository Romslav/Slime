...
local orderedStore: OrderedDataStore = DataStoreService:GetOrderedDataStore(ORDERED_NAME)
...
local function buildBoard()
    ...
    stand.Size = Vector3.new(12, 14, 0.5)
    stand.Anchored = true
    ...
    stand.CFrame = CFrame.new(center.X + radius + 20, center.Y + 5, center.Z)
    ...
    gui.CanvasSize = Vector2.new(600, 700)
...
local function renderTop(entries: { any })
    ...
    for i, entry in ipairs(entries) do
        ...
        row.BackgroundColor3 = (i == 1) and Color3.fromRGB(255, 220, 120)
        ...
        row.Text = string.format("#%d  %s  —  %s", i, entry.name or "?",
            Util.formatTime and Util.formatTime(entry.value or 0)
...
function LeaderboardService:Start()
    if not _data then _data = tryRequireSibling("DataService") end
    if not _presence then _presence = tryRequireSibling("CirclePresence") end
    if not _aura then _aura = tryRequireSibling("AuraService") end
    if not _platform then _platform = tryRequireSibling("PlatformEngine") end

    if not _platform then
        warn("[JMC][Leaderboard] PlatformEngine не найден, пропускаю постройку борда")
    else
        buildBoard()
    end
    ...
    task.spawn(function()
        -- Первый раз — через небольшую задержку, чтобы DataService успел загрузить
        task.wait(5)
        while true do
            pcall(refreshTop)
