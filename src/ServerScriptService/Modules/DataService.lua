...
do
	...
	if ok then
		...
		persistenceWarning = tostring(result)
		warn(
			"[JMC][Data] DataStore недоступен, включаю memory-only режим: " .. persistenceWarning
		)
	end
end
...
local function loadPlayer(player: Player)
	local data = nil
	if store then
		data = retryCall("GetAsync(" .. player.Name .. ")", function()
...
function DataService:Start()
	...
	local mode = persistenceEnabled and ("DataStore:" .. STORE_NAME) or "memory-only"
	print("[JMC][Data] DataService запущен (" .. mode .. ")")
end
...
function DataService:IncrementCoins(player: Player, amount: number): number
	local total = 0
...
function DataService:SpendCoins(player: Player, amount: number): boolean
	...
	self:IncrementCoins(player, -amount)
	return true
end

function DataService:SetVIP(player: Player, value: boolean)
	self:Update(player, function(d)
		d.vip = value == true
	end)
end
