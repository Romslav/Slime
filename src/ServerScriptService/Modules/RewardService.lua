...
local function getFriendIds(player: Player): { [number]: boolean }
    ...
    if ok and pages then
        ...
        while success do
            ...
            success = pcall(function() pages:AdvanceToNextPageAsync() end)
        end
    end
...
function RewardService:Start()
    ...
    getPresence().PlayerExited:Connect(function(player: Player, continuous: number)
        -- обновить HUD: показать финальное время и суммарное
        updateBillboard(player, continuous, math.floor(RewardService:GetCoins(player)))
    end)
