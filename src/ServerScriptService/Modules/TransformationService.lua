...
local function saveOriginalLook(char: Model): { [BasePart]: { Transparency: number, Material: Enum.Material, Reflectance: number, Color: Color3 } }
    local snapshot = {}
...
local function applyStage1(player: Player, state)
    ...
    task.spawn(function()
        while hl.Parent and applied[player] and applied[player].stage >= 1 do
            local up = TweenService:Create(hl, TweenInfo.new(0.9, Enum.EasingStyle.Sine),
                { FillTransparency = 0.4 })
            up:Play(); up.Completed:Wait()
            if not hl.Parent then break end
            local dn = TweenService:Create(hl, TweenInfo.new(0.9, Enum.EasingStyle.Sine),
...
local function applyStage3(player: Player, state)
    ...
    table.insert(state.instances, { Disconnect = function() if conn then conn:Disconnect() end end })
end

local function computeStage(timeSec: number): number
    if timeSec >= Config.Transformations.Stage3Time then return 3 end
    if timeSec >= Config.Transformations.Stage2Time then return 2 end
    if timeSec >= Config.Transformations.Stage1Time then return 1 end
    return 0
end
...
local function setStage(player: Player, newStage: number)
    ...
    if current == newStage then return end
    ...
    if newStage >= 1 then applyStage1(player, state) end
    if newStage >= 2 then applyStage2(player, state) end
    if newStage >= 3 then applyStage3(player, state) end
    ...
    Remotes.Event("EventBanner"):FireClient(player,
