...
local function tryBuyWithCoins(player: Player, itemId: string): (boolean, string?)
    ...
    if not price then return false, "unknown_item" end
    ...
    if not spent then return false, "spend_failed" end
    ...
    Remotes.Event("RequestPurchase").OnServerEvent:Connect(function(player, itemId, source)
        ...
        if source == "robux" then
            ...
            if pid and pid > 0 then
                ...
                Remotes.Event("PurchaseResult"):FireClient(player, {
                    ok = false, itemId = itemId, reason = "product_not_published",
                })
            end
        else
            ...
            if not ok then
                Remotes.Event("PurchaseResult"):FireClient(player, {
                    ok = false, itemId = itemId, source = "coins", reason = reason,
                })
            end
        end
    end)
        ...
        task.spawn(function()
            ...
            if vipId and vipId > 0 then
                local ok, owns = pcall(MarketplaceService.UserOwnsGamePassAsync,
                    MarketplaceService, player.UserId, vipId)
