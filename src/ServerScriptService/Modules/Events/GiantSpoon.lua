...
function E.Start(ctx)
	...
	conn = RunService.Heartbeat:Connect(function(dt)
		...
		for _, p in ipairs(ctx.presence:GetPlayersInside()) do
			if hitCooldown[p] and os.clock() < hitCooldown[p] then
				continue
			end
			...
			if
				math.abs(local_.X) < spoonLen / 2
				and math.abs(local_.Y) < spoonHeight / 2 + 2
				and math.abs(local_.Z) < spoonThick / 2 + 2
			then
				-- УДАР!
				...
				local force = lookAt * (Config.Events.GiantSpoon.HitForce or 220) + Vector3.new(0, 100, 0)
