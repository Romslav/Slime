...
function E.Start(ctx)
    ...
    if #players < 2 then
        ...
        Remotes.Event("EventBanner"):FireAllClients(
            "Нужен соперник! Зови друга.", Color3.fromRGB(180, 200, 255))
        task.wait(3)
        return
    end
    ...
    for i, p in ipairs(players) do
        ...
        if p.Character then
            ...
            if bb then table.insert(bbs, bb) end
        end
    end
    ...
    conn = RunService.Heartbeat:Connect(function(dt)
        ...
        for i = 1, #playerList do
            ...
            if not hrpA or not hrpA:IsA("BasePart") then continue end
            ...
            for j = i + 1, #playerList do
                ...
                if not charB then continue end
                local hrpB = charB:FindFirstChild("HumanoidRootPart")
                if not hrpB or not hrpB:IsA("BasePart") then continue end

                local delta = hrpB.Position - hrpA.Position
                local dist = delta.Magnitude
                if dist < 0.5 or dist > maxDist then continue end
