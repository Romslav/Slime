...
local function removeAura(player: Player)
	...
	auras[player] = nil
	if state.attach and state.attach.Parent then
...
local function updateAura(player: Player, t: number)
	...
	local rate = math.min(Config.Aura.BaseRate + t * Config.Aura.RatePerSecond, Config.Aura.MaxRate)
	pe.Rate = rate

	local size = math.min(Config.Aura.BaseSize + (t / 60) * Config.Aura.SizePerMinute, Config.Aura.MaxSize)
...
local function attachRecordBeams(player: Player)
	...
	-- Уже привязаны?
	for _, inst in ipairs(state.beamParts) do
		if typeof(inst) == "Instance" and inst:IsA("Beam") then
			return
		end
	end
	...
	if not (leftFoot:IsA("BasePart") and rightFoot:IsA("BasePart")) then
		return
	end

	local aLeft = Instance.new("Attachment")
	aLeft.Parent = leftFoot
	local aRight = Instance.new("Attachment")
	aRight.Parent = rightFoot
...
local function detachRecordBeams(player: Player)
	...
	for _, inst in ipairs(state.beamParts) do
		if
			...
			and (inst.Name == "JMC_RecordBeam" or inst.Name == "JMC_RecordBeamA" or inst.Name == "JMC_RecordBeamB")
		then
			if inst.Parent then
				inst:Destroy()
			end
