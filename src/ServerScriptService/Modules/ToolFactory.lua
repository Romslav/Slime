...
local function getCharData(player: Player): (Model?, BasePart?, Humanoid?)
	...
	if hrp and not hrp:IsA("BasePart") then
		hrp = nil
	end
	return char, hrp, hum
end
...
local function buildBanana(): Tool
	...
	tool.Activated:Connect(function()
		...
		peel.Anchored = true
		peel.CFrame = CFrame.new(hrp.Position - Vector3.new(0, 2.5, 0)) * CFrame.Angles(0, 0, math.rad(90))
		...
		peel.Touched:Connect(function(other)
			...
			if not char or char == owner.Character then
				return
			end
			...
			if not vhrp or not vhum or vhum.Health <= 0 then
				return
			end

			used = true
			local dir = edgeDirectionFromCenter(vhrp.Position)
			vhrp:ApplyImpulse(dir * cfg.EdgeImpulse * vhrp.AssemblyMass + Vector3.new(0, 60, 0) * vhrp.AssemblyMass)
			if _ragdoll then
	...
	tool.Activated:Connect(function()
		...
		if victim then
			...
			if vhrp and vhum and vhum.Health > 0 then
				...
				Remotes.Event("EventBanner"):FireClient(victim, "ЗАМОРОЖЕН!", cfg.BeamColor)
...
local function buildSlimeCannon(): Tool
	...
	tool.Activated:Connect(function()
		local now = os.clock()
		if now - lastShot < (cfg.Cooldown or 0.6) then
			return
		end
		lastShot = now
		...
		attach0.Parent = projectile
		local attach1 = Instance.new("Attachment")
		attach1.Parent = projectile
		...
		projectile.Touched:Connect(function(other)
			...
			vhrp:ApplyImpulse(
				dir.Unit * knockback * vhrp.AssemblyMass + Vector3.new(0, knockback * 0.4, 0) * vhrp.AssemblyMass
			)
...
function ToolFactory:Start()
	if not _ragdoll then
		_ragdoll = tryRequireSibling("RagdollService")
	end
