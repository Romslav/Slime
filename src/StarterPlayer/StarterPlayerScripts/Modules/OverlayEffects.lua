...
local function fxSlime(duration: number)
	...
	for i = 1, 14 do
		...
		local tw = TweenService:Create(
			drop,
			TweenInfo.new(duration * (0.6 + math.random() * 0.5), Enum.EasingStyle.Sine, Enum.EasingDirection.In),
	...
	if layer then
		task.spawn(function()
			...
			while os.clock() < endTick and layer.Parent do
				local tw = TweenService:Create(layer, TweenInfo.new(0.5), { BackgroundTransparency = 0.1 })
				tw:Play()
				tw.Completed:Wait()
				tw = TweenService:Create(layer, TweenInfo.new(0.5), { BackgroundTransparency = 0.5 })
				tw:Play()
				tw.Completed:Wait()
			end
		end)
...
local EFFECTS: { [string]: (number) -> () } = {
	...
	spicy = fxSpicy,
	spin = function(dur)
		fxSimple("spin", Color3.fromRGB(180, 140, 255), dur, 0.15)
	end,
	magnet = function(dur)
		fxSimple("magnet", Color3.fromRGB(120, 160, 255), dur, 0.1)
	end,
	float = function(dur)
		fxSimple("float", Color3.fromRGB(255, 220, 255), dur, 0.1)
	end,
	tremor = function(dur)
		fxSimple("tremor", Color3.fromRGB(255, 180, 120), dur, 0.08)
	end,
}
