...
local function doMash()
	...
	if _progress then
		...
		TweenService:Create(_progress, TweenInfo.new(0.1), { Size = UDim2.fromScale(pct, 1) }):Play()
	end
...
function ButtonMash:Start()
	Remotes.Event("EventBanner").OnClientEvent:Connect(function(text, color)
		...
		if string.find(t, "ЖМИ SPACE") or string.find(t, "ТАП!") then
			-- запуск только для баннеров Bubble Trap, а не для любых подсказок со словом SPACE
			active = true
			mashCount = 0
			buildGui()
