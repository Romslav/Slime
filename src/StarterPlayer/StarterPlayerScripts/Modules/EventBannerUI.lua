...
local function show(text: string, color: Color3?)
    ...
    local appear = TweenService:Create(_lbl, TweenInfo.new(0.4, Enum.EasingStyle.Back,
    ...
    task.delay(2.2, function()
        if not _lbl then return end
        local fade = TweenService:Create(_lbl, TweenInfo.new(0.6),
            { TextTransparency = 1 })
        fade:Play()
    end)
end
...
function EventBannerUI:Start()
    ...
    Remotes.Event("EventStarted").OnClientEvent:Connect(function(payload)
        if typeof(payload) == "table" then
            show(payload.displayName or payload.name or "СОБЫТИЕ!",
