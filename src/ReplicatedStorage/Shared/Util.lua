...
function Util.randomPointInCircle(center: Vector3, radius: number, y: number?): Vector3
	...
	return Vector3.new(center.X + math.cos(angle) * r, y or center.Y, center.Z + math.sin(angle) * r)
end

--- Точка на окружности
function Util.pointOnCircle(center: Vector3, radius: number, angle: number, y: number?): Vector3
	return Vector3.new(center.X + math.cos(angle) * radius, y or center.Y, center.Z + math.sin(angle) * radius)
end
